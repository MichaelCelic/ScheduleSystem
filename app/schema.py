...
class Query:
    ...
    @strawberry.field
    def previewSchedules(self, weekStart: date) -> List[ShiftType]:
        with get_session() as session:
            employees = session.exec(select(Employee)).all()
            locations = session.exec(select(Location)).all()
            shifts = generate_weekly_schedule(weekStart, employees, locations)
            return [ShiftType(
...
class Mutation:
    ...
    @strawberry.mutation
    def generateSchedule(self, weekStart: date) -> List[ShiftType]:
        with get_session() as session:
            employees = session.exec(select(Employee)).all()
            locations = session.exec(select(Location)).all()
            shifts = generate_weekly_schedule(weekStart, employees, locations)
